...
async function sendOrderConfirmationMail(orderItems, shippingPrice, totalPrice, fullName, address, district, commune, city, phone, email) {
  try {
    const info = await transporter.sendMail({
      ...
      html: `
        ...
        <ul style="font-size: 16px; list-style-type: none; padding: 0;">
          ${orderItems.map((item) => `
            <li style="margin-bottom: 10px;">
              ...
              Tổng: ${(item.price * item.amount - (item.price * item.discount) / 100).toLocaleString("vi-VN")}
            </li>
          `).join("")}
        </ul>
        ...
        <p style="font-size: 16px;">
          <strong>Địa chỉ: </strong> ${address + ", " + commune + ", " + district + ", " + city}<br>
